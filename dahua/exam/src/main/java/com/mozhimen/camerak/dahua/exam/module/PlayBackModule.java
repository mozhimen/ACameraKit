...
public class PlayBackModule {
    ...
    public class PlayBackDataCallBack implements CB_fDataCallBack {
        ...
        @Override
        public int invoke(long lRealHandle, int dwDataType, final byte[] buffers, int dwBufSize) {
            ToolKits.writeLog("dwBufSize = " + dwBufSize);

            beforeT = System.currentTimeMillis();
            lastT = thisT;
            thisT = System.currentTimeMillis();
            ToolKits.writeLog("callback间隔" + (thisT-lastT) + "ms");
            if(thisT-lastT > 0){
                ToolKits.writeLog("speed = " + dwBufSize/(thisT-lastT));
            } else {
                ToolKits.writeLog("speed = " + dwBufSize);
            }

//            Log.d(TAG,"PlayBackDataCallBack ");
            if (0 == dwDataType){
                IPlaySDK.PLAYInputData(mPort, buffers, buffers.length);
            }
            afterT = System.currentTimeMillis();
            ToolKits.writeLog("callback花费时长" + (afterT-beforeT) + "ms");
