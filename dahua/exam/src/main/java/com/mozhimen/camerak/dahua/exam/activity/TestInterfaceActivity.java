...
public class TestInterfaceActivity extends ListActivity {
	...
	private void generFunctionList()
	{
		...
		addFunctionToList("startSearchDevices", new ListFunctionInterface(){
			@Override
			public void listFun(){
				startSearchDevices();
			}
...
//		}
	...
	public class DevicePicUpload implements CB_fAnalyzerDataCallBack {
		...
		public void invoke(long lAnalyzerHandle, int dwAlarmType,
			...
			if(dwAlarmType == FinalVar.EVENT_IVS_ACCESS_CTL) // 门禁事件
			{
				DEV_EVENT_ACCESS_CTL_INFO trafficInfo = (DEV_EVENT_ACCESS_CTL_INFO)alarmInfo;
				ToolKits.writeLog("门禁事件");
...
//			}
	...
	public void QueryPTZCaps() {
		/// 最大页数
		CFG_PTZ_PROTOCOL_CAPS_INFO stuCaps = new CFG_PTZ_PROTOCOL_CAPS_INFO();
		char szOutBuffer[] = new char[1024];
		Integer stError = new Integer(0);
		boolean bQN = INetSDK.QueryNewSystemInfo(__LoginHandle, FinalVar.CFG_CAP_CMD_PTZ, 0, szOutBuffer, stError, 5000);
		if (bQN) {
			bQN = INetSDK.ParseData(FinalVar.CFG_CAP_CMD_PTZ, szOutBuffer, stuCaps, null);
//			ToolKits.writeLog(" = " + stuCaps.bPan);
			if (!bQN) {
...
//    	} 
	...
	/**
	 * Query Current PTZ State Info
	 */
	public void QueryPTZState()
	{
		// 1.查询设备信息
		SDK_PTZ_LOCATION_INFO PtzLocationInfo = new SDK_PTZ_LOCATION_INFO();
		boolean bRet = INetSDK.QueryDevState(__LoginHandle, FinalVar.SDK_DEVSTATE_PTZ_LOCATION, PtzLocationInfo, 5000);
		if (bRet)
		{
			ToolKits.writeErrorLog("QueryDevState SDK_DEVSTATE_PTZ_LOCATION succeed!!");
			ToolKits.writeLog("nChannelID = " + PtzLocationInfo.nChannelID);
			ToolKits.writeLog("nPTZPan = " + PtzLocationInfo.nPTZPan);
			ToolKits.writeLog("nPTZTilt = " + PtzLocationInfo.nPTZTilt);
			ToolKits.writeLog("nPTZZoom = " + PtzLocationInfo.nPTZZoom);
			ToolKits.writeLog("bZoomState = " + PtzLocationInfo.bZoomState);
			ToolKits.writeLog("nZoomValue = " + PtzLocationInfo.nZoomValue);
			ToolKits.writeLog("bFocusState = " + PtzLocationInfo.bFocusState);
			ToolKits.writeLog("fFocusPosition = " + PtzLocationInfo.fFocusPosition);
		} else {
			ToolKits.writeErrorLog("QueryDevState SDK_DEVSTATE_PTZ_LOCATION failed!!");
		}
	}
...
//    	bRet = INetSDK.OperateMonitorWall(lLoginID, emType, pInParam, pOutParam, nWaitTime);
	...
	// Obtain device information，including Device type、Hardware version、Software version、Serial number
	public void QueryDeviceSoftWare() {
		...
		if(bRet) {
			...
			ToolKits.writeLog("Device type:" + new String(version.szDevType).trim());
			ToolKits.writeLog("Hardware version:" + new String(version.szHardwareVersion).trim());
			ToolKits.writeLog("Software version:" + new String(version.szSoftWareVersion).trim());
			ToolKits.writeLog("Serial number:" + new String(version.szDevSerialNo).trim());
		} else {
   ...
   private void addMode(){
	...
	long lDevSearchHandle = 0;
	DEVICE_NET_INFO_EX deviceinfo;
	public void startSearchDevices() {

		lDevSearchHandle = INetSDK.StartSearchDevices(new Test_SearchDevicesCB1());
		if(lDevSearchHandle == 0) {
			ToolKits.writeErrorLog("StartSearchDevices Failed!" + INetSDK.GetLastError());
		} else {
			ToolKits.writeLog("after Start, Search Handle = " + lDevSearchHandle);
		}


//		DEVICE_IP_SEARCH_INFO stIpInfo = new DEVICE_IP_SEARCH_INFO();
//		stIpInfo.nIpNum = 1;
//		System.arraycopy("192.168.1.107".getBytes(), 0, stIpInfo.szIP[0], 0,
//				"192.168.1.107".getBytes().length);
//
//		if(INetSDK.SearchDevicesByIPs(stIpInfo, new Test_SearchDevicesCB1(), 3000)){
//			ToolKits.writeLog("SearchDevicesByIPs succeed");
//		} else {
//			ToolKits.writeErrorLog("SearchDevicesByIPs failed");
//		}
	}

	public class Test_SearchDevicesCB1 implements CB_fSearchDevicesCB{
		int cnt = 0;
		@Override
		public void invoke(final DEVICE_NET_INFO_EX device_net_info_ex) {
			ToolKits.writeLog("Device[" + ++cnt + "]");
			ToolKits.writeLog("szMac = " + new String(device_net_info_ex.szMac).trim());
			ToolKits.writeLog("szSerialNo = " + new String(device_net_info_ex.szSerialNo).trim());
			ToolKits.writeLog("szIP = " + new String(device_net_info_ex.szIP).trim());
			ToolKits.writeLog("szGateway = " + new String(device_net_info_ex.szGateway).trim());
			ToolKits.writeLog("szSubmask = " + new String(device_net_info_ex.szSubmask).trim());
			ToolKits.writeLog("szMac = " + new String(device_net_info_ex.szMac).trim());
			ToolKits.writeLog("bDhcpEn = " + device_net_info_ex.bDhcpEn);
			if(new String(device_net_info_ex.szSerialNo).trim().equals("7G036A1PCG21324")){
				deviceinfo = device_net_info_ex;
				new Thread(new Runnable() {
					@Override
					public void run() {
						ToolKits.writeLog("before stop,Search Handle = " + lDevSearchHandle);
						if(!INetSDK.StopSearchDevices(lDevSearchHandle)){
							ToolKits.writeErrorLog("StopSearchDevices Failed!");
						}
					}
				}).start();
			}
	...
	public void TestModifyDevice(){
		NET_IN_MODIFY_IP stIn = new NET_IN_MODIFY_IP();
		NET_OUT_MODIFY_IP stOut = new NET_OUT_MODIFY_IP();
		String username = "admin";
		String pwd = "admin123";
		String ip = "192.168.1.107";
		System.arraycopy(username.getBytes(), 0, deviceinfo.szUserName, 0, username.getBytes().length);
		System.arraycopy(pwd.getBytes(), 0, deviceinfo.szPassWord, 0, pwd.getBytes().length);
		System.arraycopy(username.getBytes(), 0, deviceinfo.szNewUserName, 0, username.getBytes().length);
		System.arraycopy(pwd.getBytes(), 0, deviceinfo.szNewPassWord, 0, pwd.getBytes().length);
		deviceinfo.nPort = 37777;
		deviceinfo.bDhcpEn = !deviceinfo.bDhcpEn;
		stIn.stuDevNetInfo.stuDevInfo = deviceinfo;
		System.arraycopy(ip.getBytes(), 0, stIn.stuDevNetInfo.szLocalIP, 0, ip.getBytes().length);

		ToolKits.writeLog(deviceinfo.toString());
		if(INetSDK.ModifyDevice(deviceinfo, 5000)){
			ToolKits.writeLog("ModifyDevice Succeed !");
		} else {
			ToolKits.writeErrorLog("ModifyDevice failed !");
		}
//		if(INetSDK.ModifyDeviceEx(stIn, stOut,5000)){
//			ToolKits.writeLog("ModifyDeviceEx Succeed !");
//		} else {
//			ToolKits.writeErrorLog("ModifyDeviceEx failed !");
//		}
	}
