...
import com.mozhimen.camerak.dahua.exam.module.MarkRecordModule;
...
//        recyclerView.addItemDecoration(itemDecoration);
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.mark_record_start_date || id == R.id.mark_record_start_time) {
            selectMarkTime(MARK_START_DATE_TIME);
        } else if (id == R.id.mark_record_end_date || id == R.id.mark_record_end_time) {
            selectMarkTime(MARK_END_DATE_TIME);
        } else if (id == R.id.lock_record_button) {
            emRecordType = MARK_RECORD_TYPE.MARK_RECORD_TYPE_LOCK;
            new MarkRecordTask().execute();
        } else if (id == R.id.query_record_button) {
            emRecordType = MARK_RECORD_TYPE.MARK_RECORD_TYPE_QUERY;
            new MarkRecordTask().execute();
        } else if (id == R.id.unlock_record_button) {
            if (mFileList.size() == 0) {
