package com.mozhimen.camerak.opengl.proxy.bases
...
//        const val BASEFILTER_KEY_INDEX = 0x201
    ...
    protected val originalMatrix: FloatArray = MatrixUtil.get_original()//单位矩阵
    ...
    protected open fun createProgramByAssetsFile(vertex: String, fragment: String) {
        createProgram(
            UtilKStrAsset.strAssetName2str_use_ofStream(vertex)?:"",
            UtilKStrAsset.strAssetName2str_use_ofStream(fragment)?:""
        )
    }

    protected open fun createProgram(vertex: String, fragment: String) {
        program = GLES20Util.createGlProgram(vertex, fragment)
