...
import com.mozhimen.camerak.opengl.proxy.commons.ICameraKFrameListener
import com.mozhimen.camerak.opengl.proxy.commons.ICameraKRenderer
import com.mozhimen.kotlin.utilk.android.graphics.UtilKMatrix
import com.mozhimen.kotlin.elemk.android.opengl.annors.AMatrixType
import com.mozhimen.openglk.basic.utils.GLES20Util
import com.mozhimen.openglk.basic.utils.MatrixUtil
...
class GLSurfaceRenderer(context: Context) : GLSurfaceView.Renderer {
    ...
    private var _camera2KRenderer: ICameraKRenderer? = null//用户附加的Renderer或用来监听Renderer
    ...
    private val _screenMatrix: FloatArray = MatrixUtil.get_original() //用于绘制到屏幕上的变换矩阵
    private var _showType: Int = AMatrixType.MATRIX_CENTER_CROP //输出到屏幕上的方式
    ...
    private var _camera2KFrameListener: ICameraKFrameListener? = null//回调
    ...
    fun setCamera2KRenderer(renderer: ICameraKRenderer) {
    ...
    fun setFrameListener(width: Int, height: Int, listener: ICameraKFrameListener) {
        _frameWidth = width
        _frameHeight = height
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        GLES20Util.genTexturesAndParameterf(
    ...
    private fun calculateCallbackOM() {
        if (_frameHeight > 0 && _frameWidth > 0 && _inputPreviewSize!!.x > 0 && _inputPreviewSize!!.y > 0) {
            ...
            MatrixUtil.apply(
                ...
                AMatrixType.MATRIX_CENTER_CROP, _inputPreviewSize!!.x, _inputPreviewSize!!.y,
                _frameWidth,
                _frameHeight
            )
            com.mozhimen.kotlin.utilk.android.opengl.UtilKMatrix.flip_ofXY(_callbackOptionMatrix, isFlipX = false, isFlipY = true)
        }
    }
    ...
    private fun callbackIfNeeded() {
        if (_camera2KFrameListener != null && (_isRecord || _isShoot)) {
            ...
            GLES20Util.bindFrameBuffer(_exportFrame[0], _exportTexture[0])
            _showFilter?.setMatrix(_callbackOptionMatrix)
            _showFilter?.draw()
            onFrameCallback()
            _isShoot = false
            GLES20Util.unBindFrameBuffer()
            _showFilter?.setMatrix(_screenMatrix)
        }
    }
