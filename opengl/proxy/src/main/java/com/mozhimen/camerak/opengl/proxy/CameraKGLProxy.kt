package com.mozhimen.camerak.opengl.proxy
...
import com.mozhimen.camerak.opengl.proxy.helpers.GLSurfaceRenderer
 ...
 * @Version 1.0
 */
class CameraKGLProxy(activity: Activity, renderer: GLSurfaceRenderer) : IUtilK {
    ...
    fun openCamera(previewWidth: Int, previewHeight: Int) {
        require(UtilKPermission.isSelfGranted(CPermission.CAMERA)) { "$TAG need camera permission" }
        ...
        try {
            ...
            e.printStackTrace()
            Log.e(TAG, "openCamera: fail CameraAccessException ${e.message}")
            "请检查摄像头".showToastOnMain()
        } catch (e: IllegalArgumentException) {
            e.printStackTrace()
            Log.e(TAG, "openCamera: fail IllegalArgumentException ${e.message}")
            "请检查摄像头".showToastOnMain()
        }
    }
    ...
    private fun getRotation(activity: Activity): Int {
        var displayRotation = UtilKScreen.getRotation(activity)
